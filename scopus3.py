...
if __name__ == '__main__':
    ...
    data_Ori = pd.read_excel(url_static+'Link_file.xlsx')
    ...
    k = 1
    outcome = []
    for lastName, firstName, aff in data_Ori[['last_name', 'first_name', 'school']].values[510:518]:
        print('[%d]:' % k, end=' ')
        k += 1
        aff = schoolMap[aff]
        ...
        try:
            ...
            for item in resJSON['search-results']['entry']:
                # if item['affiliation-current']['affiliation-name'].lower() == aff.lower() and aff != 'University of South Florida':
                if similar(aff.lower(), item['affiliation-current']['affiliation-name'].lower()) > 0.5:
                    authorID = item['dc:identifier'].split(':')[-1]
                    # print(item['subject-area'][0]['$'].split(',')[0])
                    break
                # elif similar(aff.lower(), item['affiliation-current']['affiliation-name'].lower()) > 0.5:
                #     authorID = resJSON['search-results']['entry'][0]['dc:identifier'].split(':')[-1]
                #     break
                else:
                    print(similar(aff.lower(), item['affiliation-current']['affiliation-name'].lower()))
                    break
                    # raise ValueError
            # print(resJSON['search-result']['entry'][0]['subject-area'].split(':')[-1].split(','))
        ...
        # else:
        #     authorID = lastName = firstName = articalAuthors = articalYear = articalName = articalPub = articalCite = aff = num = 'none'
    ...
    outcome = pd.DataFrame(outcome, columns=['AuthorID', 'Last Name', 'First Name', 'All Authors', 'Year', 'Title',
                                             'Journal Name', 'Citation', 'Affiation', 'outcomeNum'])
    path = '/Users/leono1/OneDrive/Research_2019/Jared_Project/'
    outcome.to_csv(path + 'USF.csv')
